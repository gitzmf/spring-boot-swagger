...
SpringBoot启动应用类上增加@EnableSwagger2注解即可，但是通常我们增加到Swagger的Config配置类上
方便我们对Swagger API文档的使用环境进行控制，例如，我们只在开发和测试环境下使用，其他环境关闭
我们只需要配置类上增加@Profile({"dev","test"})即可
    1> Swagger配置类上
        @Configuration
...
如果需要配置多个分组
    配置多个docket,每个docket中groupName设置不同的组名即可
